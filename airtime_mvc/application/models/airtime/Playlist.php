...
abstract class Playlist extends BasePlaylist implements \Interface_Playlistable
{
	...
	public function getTimezone() {
        ...
        return $rules[$this::RULE_TIMEZONE];
	}
	...
	protected function getCriteriaRules(&$query) {
		...
		$utcConvertCols = array("CreatedAt", "UpdatedAt", "LastPlayedTime");
		...
		*/
		...
		$createRule = function (&$query, $comparison, $pattern, $utcConvertCols, $playlistTimezone, $utcTimezone) {
			return function($col, $data) use (&$query, $comparison, $pattern, $utcConvertCols, $playlistTimezone, $utcTimezone) {
				...
				if (in_array($col, $utcConvertCols)) {
                    $dt = new Datetime($value1, $playlistTimezone);
                    $dt->setTimezone($utcTimezone);

                    $value1 = $dt->format('Y-m-d H:i:s');
				}
		...
		$range = function ($col, $data) use (&$query, $utcConvertCols, $playlistTimezone, $utcTimezone) {
			...
			if (in_array($col, $utcConvertCols)) {
			    ...
			    $dt = new Datetime($value2, $playlistTimezone);
			    $dt->setTimezone($utcTimezone);

			    $value2 = $dt->format('Y-m-d H:i:s');
			}
    ...
    public function generateContent(PropelPDO $con) {
    	...
    	try {
    		...
    		//only look at fully imported files.
    		$query->filterByImportStatus(0);
